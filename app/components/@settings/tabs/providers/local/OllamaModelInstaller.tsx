    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between pt-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-artify-elements-textPrimary">Ollama Models</h3>
            <p className="text-sm text-artify-elements-textSecondary mt-1">Install and manage your Ollama models</p>
          </div>
        </div>
        <motion.button
          ...
          >
            ...
            <div className="flex-1 space-y-1.5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-artify-elements-textPrimary font-mono text-sm">{model.name}</p>
                  <p className="text-xs text-artify-elements-textSecondary mt-0.5">{model.desc}</p>
                </div>
                <div className="text-right">
                  <span className="text-xs text-artify-elements-textTertiary">{model.size}</span>
                  {model.installedVersion && (
                    <div className="mt-0.5 flex flex-col items-end gap-0.5">
                      <span className="text-xs text-artify-elements-textTertiary">v{model.installedVersion}</span>
                      {model.needsUpdate && model.latestVersion && (
                        <span className="text-xs text-green-500">v{model.latestVersion} available</span>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap gap-1">
                  {model.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-1.5 py-0.5 rounded-full text-[10px] bg-artify-elements-background-depth-4 text-artify-elements-textTertiary"
                    >
                      {tag}
                    </span>
                  ))}
      ...
      {installProgress && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          <div className="flex justify-between text-sm">
            <span className="text-artify-elements-textSecondary">{installProgress.status}</span>
            <div className="flex items-center gap-4">
              <span className="text-artify-elements-textTertiary">
                {installProgress.downloadedSize} / {installProgress.totalSize}
              </span>
              <span className="text-artify-elements-textTertiary">{installProgress.speed}</span>
              <span className="text-artify-elements-textSecondary">{Math.round(installProgress.progress)}%</span>
            </div>
          </div>
