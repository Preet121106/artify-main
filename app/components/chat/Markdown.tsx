      ...
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes("__artifySelectedElement__")) {
            ...
            return (
              <div className="bg-artify-elements-background-depth-3 border border-artify-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-artify-elements-background-depth-2 px-2 py-1 rounded text-artify-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-artify-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-artify-elements-textSecondary !bg-artify-elements-background-depth-2 border border-artify-elements-borderColor p-2 rounded">
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-artify-elements-item-backgroundAccent text-artify-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
 ...
 * const input = "```xml\n<div class='__artifyArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__artifyArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __artifyArtifact__ class)
