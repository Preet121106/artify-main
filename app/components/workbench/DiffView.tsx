...
const FullscreenOverlay = memo(({ isFullscreen, children }: { isFullscreen: boolean; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-6">
      <div className="w-full h-full max-w-[90vw] max-h-[90vh] bg-artify-elements-background-depth-2 rounded-lg border border-artify-elements-borderColor shadow-xl overflow-hidden">
        {children}
      </div>
    </div>
  );
  ...
  ({
    ...
    <div className="h-full flex flex-col items-center justify-center p-4">
      <div className="text-center text-artify-elements-textTertiary">
        ...
        <p className="font-medium text-artify-elements-textPrimary">Files are identical</p>
        <p className="text-sm mt-1">Both versions match exactly</p>
      </div>
      <div className="mt-4 w-full max-w-2xl bg-artify-elements-background-depth-1 rounded-lg border border-artify-elements-borderColor overflow-hidden">
        <div className="p-2 text-xs font-bold text-artify-elements-textTertiary border-b border-artify-elements-borderColor">
          Current Content
        </div>
...
const FileInfo = memo(
  ({
    ...
    return (
      <div className="flex items-center bg-artify-elements-background-depth-1 p-2 text-sm text-artify-elements-textPrimary shrink-0">
        ...
        <span className="ml-auto shrink-0 flex items-center gap-2">
          {hasChanges ? (
            <>
              ...
              <span className="text-artify-elements-textTertiary text-xs">{new Date().toLocaleTimeString()}</span>
            </>
          ) : (
  ...
  if (!highlighter) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-artify-elements-textTertiary">Loading diff...</div>
      </div>
    );
  }
