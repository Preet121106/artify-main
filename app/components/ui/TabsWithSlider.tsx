...
export function TabsWithSlider({
  ...
  return (
    <div className={classNames('relative flex gap-2', className)}>
      ...
      {tabs.map((tab, index) => (
        <button
          ...
          className={classNames(
            ...
            tab.id === activeTab
              ? classNames('text-white shadow-sm shadow-green-500/20', activeTabClassName)
              : classNames(
      ...
      {/* Animated slider */}
      <motion.div
        className={classNames('absolute bottom-0 left-0 h-10 rounded-lg bg-green-500 -z-10', sliderClassName)}
        initial={false}
        animate={{
