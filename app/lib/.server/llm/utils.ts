...
export function simplifyartifyActions(input: string): string {
  // Using regex to match artifyAction tags that have type="file"
  const regex = /(<artifyAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/artifyAction>)/g;
