...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyartifyActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyartifyActions(content);

      content = content.replace(/<div class=\\"__artifyThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`artify failed to select files`);
  }
