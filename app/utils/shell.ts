...
export class artifyShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newartifyShellProcess(webcontainer, terminal);
  ...
  async newartifyShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn("/bin/jsh", ["--osc", ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
...
export function newartifyShellProcess() {
  return new artifyShell();
}
