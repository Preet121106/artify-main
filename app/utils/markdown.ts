  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === "html" && node.value && node.value.startsWith("<think>")) {
        ...
        node.value = `<div class="__artifyThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      "data*",
      ["className", "__artifyArtifact__", "__artifyThought__", "__artifyQuickAction", "__artifySelectedElement__"],

      // ['className', '__artifyThought__']
    ],
    button: [
      ...
      "name",
      "value",
      ["className", "__artifyArtifact__", "__artifyThought__", "__artifyQuickAction"],
    ],
  },
  strip: [],
};
